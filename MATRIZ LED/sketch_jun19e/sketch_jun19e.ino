...
void animateChar( ) {
  ...
  String frase = "Hola me gusta tu madre";

  for ( int i = 0; i < frase.length() * 7; i++ ) {
    // limpiar la matriz
    mx.clear();
    // dibujar el caracter c empezando en la columna i
    mx.setChar(i, frase.charAt(0));
    for ( int s = 1; s < frase.length(); s++) {

      if (frase.charAt(s - 1) == 'l') {
        mx.setChar(i + (s * (-6) + 3), frase.charAt(s));
      }

      else if (frase.charAt(s - 1) == 't') {
        mx.setChar(i + (s * (-6) + 2), frase.charAt(s));
      }

      else
        mx.setChar(i + (s * (-6)), frase.charAt(s));
    }
    // esperar un tiempo
    delay(150);
